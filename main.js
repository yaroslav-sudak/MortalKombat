...
function generateLogs(type, player1, player2){
	switch (type){
		case 'start':
			...
			const timeS = dateS.getHours() + ':' + minuteS;
			let textS = logs['start'].replace('[player1]', player1.name).replace('[player2]', player2.name).replace('[time]', timeS);
			const elS = `<p>${textS}</p>`;
			$chat.insertAdjacentHTML('afterbegin', elS);
			break;
		default:
			const date = new Date;
			...
			let text = logs[type][getRandom(logs[type].length)].replace('[playerKick]', player1.name).replace('[playerDefence]', player2.name);
